...
def change_finetune_model_params(ckpt, finetune_model, model_config, multi_task=False, model_branch=''):
    ...
    if finetune_model is not None:
        ...
        if not finetune_multi_task:
            ...
            new_fitting = False
            if model_branch == '':
                model_branch_chosen = list(model_dict_params.keys())[0]
                new_fitting = True
                model_config['bias_shift'] = 'statistic'  # fitting net re-init
                print('The fitting net will be re-init instead of using that in the pretrained model! '
            ...
            for key_item in ['type_map', 'type_embedding', 'descriptor']:
                if key_item in model_dict_params[model_branch_chosen]:
                    model_config[key_item] = model_dict_params[model_branch_chosen][key_item]
            if not new_fitting:
                model_config['fitting_net'] = model_dict_params[model_branch_chosen]['fitting_net']
            logging.info(f"Change the model configurations according to the model branch "
