...
def preprocess_shared_params(model_config):
    ...
    def replace_one_item(params_dict, key_type, key_in_dict, suffix='', index=None):
        ...
        if ":" in key_in_dict:
            ...
            shared_level = key_in_dict.split(":")[1]
                ...
                if isinstance(item_params, str):
                    ...
                    for ii, hybrid_item in enumerate(item_params["list"]):
                        if isinstance(hybrid_item, str):
                            replace_one_item(model_params_item[item_key]["list"], item_key, hybrid_item,
                                             suffix=f"_hybrid_{ii}", index=ii)
    for shared_key in shared_links:
        shared_links[shared_key]["links"] = sorted(shared_links[shared_key]["links"], key=lambda x: x['shared_level'])
...
def get_class_name(item_key, item_params):
    if item_key == "type_embedding":
        ...
        elif item_type == "se_atten":
            return DescrptSeAtten.__name__
        elif item_type == "se_uni":
            return DescrptSeUni.__name__
