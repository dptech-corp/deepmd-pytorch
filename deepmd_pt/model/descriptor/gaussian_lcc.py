...
from deepmd_pt.model.network import TypeFilter, TypeEmbedNet, GaussianEmbedding, Evoformer3bEncoder
...
class DescrptGaussianLcc(Descriptor):
    ...
    def __init__(self,
        ...
        """
        super(DescrptGaussianLcc, self).__init__()
        self.rcut = rcut
        ...
        self.edge_type_embedding = nn.Embedding((ntypes + 1) * (ntypes + 1),
                                                pair_embed_dim, padding_idx=(ntypes + 1) * (ntypes + 1) - 1,
                                                dtype=env.GLOBAL_PT_FLOAT_PRECISION)
        self.gaussian_encoder = GaussianEmbedding(rcut, kernel_num, num_pair, embed_dim,
                                                  pair_embed_dim, sel, ntypes, atomic_sum_gbf)
        self.backbone = Evoformer3bEncoder(self.nnei,
    ...
    def forward(self,
        ...
        nlist2 = torch.cat(
            [torch.arange(0, nloc, device=nlist.device).reshape(1, nloc, 1).expand(nframes, -1, -1),
             nlist], dim=-1)
        nlist_loc2 = torch.cat(
            [torch.arange(0, nloc, device=nlist_loc.device).reshape(1, nloc, 1).expand(nframes, -1, -1),
             nlist_loc], dim=-1)
        ...
        attn_mask = torch.zeros([nframes, nloc, 1 + self.nnei, 1 + self.nnei, self.attention_heads],
                                device=nlist.device,
        ...
        if self.pre_add_seq and seq_input is not None:
            ...
            if first_dim == nframes * nloc:
                ...
                raise RuntimeError
        atom_feature = atom_feature * nnei2_mask.reshape(nframes * nloc, 1 + self.nnei, 1)

        # Pair feature
        # [(nframes x nloc) x (1 + nnei2)]
        nlist_type2_reshape = nlist_type2.reshape(nframes * nloc, 1 + self.nnei)
        # [(nframes x nloc) x (1 + nnei2) x (1 + nnei2)]
        edge_type = nlist_type2_reshape.unsqueeze(-1) * (self.ntypes + 1) + nlist_type2_reshape.unsqueeze(-2)
        ...
        edge_type_2dim = torch.cat(
            [nlist_type2_reshape.view(nframes * nloc, 1 + self.nnei, 1, 1).expand(-1, -1, 1 + self.nnei, -1),
             nlist_type2_reshape.view(nframes * nloc, 1, 1 + self.nnei, 1).expand(-1, 1 + self.nnei, -1,
                                                                                  -1) + self.ntypes],
            dim=-1,
        )
        # [(nframes x nloc) x (1 + nnei2) x 3]
        coord_selected = torch.gather(
            extended_coord.unsqueeze(1).expand(-1, nloc, -1, -1).reshape(nframes * nloc, nall, 3),
            dim=1, index=nlist2.reshape(nframes * nloc, 1 + self.nnei, 1).expand(-1, -1, 3))

        # Update pair features (or and atomic features) with gbf features
        # delta_pos: [(nframes x nloc) x (1 + nnei2) x (1 + nnei2) x 3].
        atomic_feature, pair_feature, delta_pos = self.gaussian_encoder(coord_selected, atom_feature,
                                                                        edge_type_2dim, edge_feature)
        # [(nframes x nloc) x (1 + nnei2) x (1 + nnei2) x pair_dim]
        attn_bias = pair_feature

        # output: [(nframes x nloc) x (1 + nnei2) x tebd_dim]
        # pair: [(nframes x nloc) x (1 + nnei2) x (1 + nnei2) x pair_dim]
        output, pair = self.backbone(atomic_feature, pair=attn_bias, attn_mask=attn_mask, pair_mask=pair_mask,
