...
class DescrptDPA2(Descriptor):
  def __init__(
      ...
      # ffn, ffn_embed_dim, scaling_factor, normalize, temperature
    ...
    self.repformer = DescrptSeUni(
      ...
      axis_neuron=repformer_axis_dim,
    ...
    if self.repinit.dim_out == self.repformer.dim_in:
      self.g1_shape_tranform = Identity()
    else:
      self.g1_shape_tranform = Linear(
        self.repinit.dim_out, 
        self.repformer.dim_in,
        bias=False, 
        init="glorot",
      )
    assert self.repinit.rcut > self.repformer.rcut
    assert self.repinit.sel > self.repformer.sel    
  ...
  def compute_input_stats(self, merged):
      ...
      for ii, descrpt in enumerate([self.repinit, self.repformer]):
  ...
  def init_desc_stat(self, merged):
  ...
  def forward(
    ...
    assert extended_atype_embd is None
    ...
    nlist_dict = build_multiple_neighbor_list(
      extended_coord,
      nlist,
      [self.repformer.rcut, self.repinit.rcut],
      [self.repformer.sel[0], self.repinit.sel[0]],
    )
    # repinit
    g1_ext = self.type_embedding(extended_atype)
    g1, env_mat, diff, rot_mat, sw = self.repinit(
      nlist_dict[self.repinit.rcut],
      extended_coord,
      extended_atype,
      g1_ext, mapping)
    ...
    # repformer
    g1, env_mat, diff, rot_mat, sw = self.repformer(
      nlist_dict[self.repformer.rcut],
      extended_coord,
      extended_atype,
      g1_ext, mapping)
