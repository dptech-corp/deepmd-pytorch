...
class MockDescriptor:
    def compute_input_stats(self, sampled):
        return [[]] * 5
...
def convert_batch_to_pyg(coord, atype, box, pad_index, selected=None, mapping=None, shifts=None):
    bsz, padded_natom = coord.shape[:2]
    nnei = selected.size(-1)

    mask = atype != pad_index

    data = Data()
    data.atomic_numbers = atype[mask] + 1  # ocpmodels use 1-based atomic numbers
    data.pos = coord[mask].float()  # TODO: confirm the dtype here

    # TODO: check this - row/column vector?
    data.cell = box.reshape(bsz, 3, 3).float()  # TODO: confirm the dtype here

    data.natoms = mask.long().sum(dim=1)
    data.ptr = torch.cat([
        torch.zeros(1).to(data.natoms),
        torch.cumsum(data.natoms, dim=0),
    ], dim=0)
    data.batch = (
        torch.arange(bsz).to(data.natoms).repeat_interleave(data.natoms)
    )

    if selected is not None:
        assert mapping is not None and shifts is not None

        pbc_atom_mask = mapping != -1
        pbc_edge_mask = selected != -1

        selected += data.ptr[:-1].view(-1, 1, 1)

        pbc_src = selected[pbc_edge_mask]

        mapping += data.ptr[:-1].view(-1, 1)
        mapping = mapping[pbc_atom_mask]
        edge_src = mapping[pbc_src]
        edge_dst = (
            torch.arange(padded_natom).to(data.natoms)
                 .view(1, -1, 1).expand(bsz, -1, nnei)
        )[pbc_edge_mask]

        data.edge_index = torch.stack([edge_dst, edge_src], dim=0)
        data.cell_offsets = torch.linalg.solve(
            data.cell.repeat_interleave(pbc_atom_mask.long().sum(dim=1), dim=0),
            shifts[pbc_atom_mask].to(data.cell),
        )[pbc_src]
        data.neighbors = pbc_edge_mask.long().sum(dim=[1, 2])
...
def radius_graph_pbc(
    ...
    cells_per_dim = [
        torch.arange(-rep, rep + 1, device=device, dtype=atom_pos.dtype)
        for rep in max_rep
    ]
...
def get_max_neighbors_mask(
    ...
    distance_sort = torch.full(
        [num_atoms * max_num_neighbors], np.inf, device=device, dtype=atom_distance.dtype
    )
