...
from deepmd_pt.model.model import BaseModel as DPABaseModel

from ..base import OCPBaseModel
from ..utils import convert_batch_to_pyg, MockDescriptor
...
from .scale_factor import load_scales_compat


class GemNetTModel(DPABaseModel):
    """
    NOTE:
        In the config, both model_params['param']['otf_graph'] and model_params['descriptor']
        controls the graph construction.
        The logic is:
        1. if otf_graph is True: ignore the default graph in DPA and use on-the-fly graph in
        ocpmodels.base.BaseModel, which is the default way in OC20.
        2. otherwise: use the default graph in DPA, which is built according to params
        specified in model_params['descriptor'].
    """
    def __init__(self, model_params, sampled=None):
        ...
        self.net = GemNetT(**model_params['params'])

        self.ntypes = len(model_params['type_map'])
        fitting_param = {}
        self.compute_or_load_stat(model_params, fitting_param, self.ntypes, sampled=sampled)
        self.bias_atom_e = torch.nn.Parameter(
            torch.tensor(fitting_param['bias_atom_e']),
            requires_grad=False,
        )

    def forward(self, coord, atype, natoms, mapping, shifts, selected, selected_type, selected_loc=None, box=None):
        data = convert_batch_to_pyg(
            coord=coord,
            atype=atype,
            box=box,
            pad_index=self.ntypes,
            selected=selected if not self.net.otf_graph else None,
            mapping=mapping if not self.net.otf_graph else None,
            shifts=shifts if not self.net.otf_graph else None,
        )

        if self.net.regress_forces:
            energy, force_pyg = self.net(data)
            force = torch.zeros_like(coord).to(force_pyg)
            force[atype != self.ntypes] = force_pyg
        else:
            energy = self.net(data)

        reference_energy = scatter(
            self.bias_atom_e[data.atomic_numbers - 1],  # DPA use 0-based numbers
            data.batch,
        )
        energy += reference_energy.to(energy).unsqueeze(-1)
...
class GemNetT(OCPBaseModel):
    ...
    def __init__(
        ...
        scale_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'gemnet-dT.json')
