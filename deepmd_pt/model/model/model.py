...
class BaseModel(torch.nn.Module):
    ...
    def compute_or_load_stat(self, fitting_param, ntypes,
        ...
        if 'prop' in fitting_type:
            self.compute_or_load_stat_prop(fitting_param, ntypes, resuming, type_map, stat_file_dir, stat_file_path, sampled)
        else:
            self.compute_or_load_stat_ener(fitting_param, ntypes, resuming, type_map, stat_file_dir, stat_file_path, sampled)

    
    def compute_or_load_stat_ener(self, fitting_param, ntypes,
                             resuming=False, type_map=None, stat_file_dir=None, stat_file_path=None, sampled=None):
        if not resuming:
            if sampled is not None:  # compute stat
                ...
                if not isinstance(stat_file_path, list):
                    ...
                    assert not missing_type, \
                        f"These type are not in stat file {stat_file_path}: {missing_type}! Please change the stat file path!"
    ...
    def compute_or_load_stat_prop(self, fitting_param, ntypes,
                            resuming=False, type_map=None, stat_file_dir=None, stat_file_path=None, sampled=None):
        if not resuming:
            if sampled is not None:  # compute stat
                for sys in sampled:
                ...
                if stat_file_path is not None:
                    ...
                    if not isinstance(stat_file_path, list):
                        ...
                        np.savez_compressed(stat_file_path,
                                            sumr=sumr, suma=suma, sumn=sumn, sumr2=sumr2, suma2=suma2,
                                            bias_atom_p=fitting_param['bias_atom_p'], type_map=type_map)
                    else:
                        for ii, file_path in enumerate(stat_file_path):
                            logging.info(f'Saving stat file to {file_path}')
                            np.savez_compressed(file_path,
                                                sumr=sumr[ii], suma=suma[ii], sumn=sumn[ii], sumr2=sumr2[ii], suma2=suma2[ii],
                                                bias_atom_p=fitting_param['bias_atom_p'],
                                                type_map=type_map)
            else:  # load stat
                target_type_map = type_map
                if not isinstance(stat_file_path, list):
                    ...
                    if stats["sumr"].size:
                        ...
                        sumr, suma, sumn, sumr2, suma2 = [], [], [], [], []
                    fitting_param['bias_atom_p'] = stats["bias_atom_p"][idx_map]
                else:
                    sumr, suma, sumn, sumr2, suma2 = [], [], [], [], []
                    id_bias_atom_p = None
                    for ii, file_path in enumerate(stat_file_path):
                        ...
                        suma2.append(suma2_tmp)
                        fitting_param['bias_atom_p'] = stats["bias_atom_p"][idx_map]
                        if id_bias_atom_p is None:
                            id_bias_atom_p = fitting_param['bias_atom_p']
                        else:
                            assert (id_bias_atom_p == fitting_param['bias_atom_p']).all(), \
                                "bias_atom_p in stat files are not consistent!"
            self.descriptor.init_desc_stat(sumr, suma, sumn, sumr2, suma2)
        else:  # resuming for checkpoint; init model params from scratch
