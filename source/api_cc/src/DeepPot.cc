...
void DeepPot::compute(ENERGYVTYPE &ener, std::vector<VALUETYPE> &force,
  ...
  // make_env_mat(nlist, nlist_loc, nlist_type, merged_coord_shift,
  // merged_mapping, coord_wrapped, coord, atype, box, rcut, sec);
  ...
  // coord_wrapped[3*ii+0], coord_wrapped[3*ii+1], coord_wrapped[3*ii+2]); }
  // printf("\n");
  ...
  force.assign(cpu_force_.data_ptr<double>(),
               cpu_force_.data_ptr<double>() + cpu_force_.numel());
...
template void
DeepPotModelDevi::init<double>(const std::vector<std::string> &models);
...
void DeepPotModelDevi::compute(std::vector<ENERGYVTYPE> &all_energy,
  ...
  for (int ii = 0; ii < numb_models; ii++) {
    c10::Dict<c10::IValue, c10::IValue> outputs =
        modules[ii].forward(inputs).toGenericDict();
    ...
    torch::Tensor flat_force_ = force_.toTensor().view({-1});
    torch::Tensor cpu_force_ = flat_force_.to(torch::kCPU);
    all_force[ii].assign(cpu_force_.data_ptr<double>(),
                         cpu_force_.data_ptr<double>() + cpu_force_.numel());

    torch::Tensor flat_virial_ = virial_.toTensor().view({-1});
    torch::Tensor cpu_virial_ = flat_virial_.to(torch::kCPU);
    all_virial[ii].assign(cpu_virial_.data_ptr<double>(),
                          cpu_virial_.data_ptr<double>() + cpu_virial_.numel());
  }
}
template void DeepPotModelDevi::compute<double, double>(
...
template void
DeepPotModelDevi::compute_avg<double>(double &dener,
                                      const std::vector<double> &all_energy);

template void
DeepPotModelDevi::compute_avg<float>(float &dener,
...
template void
DeepPotModelDevi::compute_avg<float>(std::vector<float> &avg,
                                     const std::vector<std::vector<float>> &xx);

template <typename VALUETYPE>
void DeepPotModelDevi::compute_std(
    std::vector<VALUETYPE> &std, const std::vector<VALUETYPE> &avg,
...
template void DeepPotModelDevi::compute_std<double>(
    std::vector<double> &std, const std::vector<double> &avg,
    const std::vector<std::vector<double>> &xx, const int &stride);

template void DeepPotModelDevi::compute_std<float>(
    std::vector<float> &std, const std::vector<float> &avg,
    const std::vector<std::vector<float>> &xx, const int &stride);
