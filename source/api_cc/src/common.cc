...
int deepmd::torch_input_tensors(
  ...
  std::vector<int> natoms_vector(2 + ntypes, 0);
  natoms_vector(0) = nloc;
  natoms_vector(1) = nall;
  for (int ii = 0; ii < ntypes; ++ii) {
    natoms_vector(ii + 2) = type_count[ii];
  }
  datype.insert(datype.end(), datype_.begin() + nloc, datype_.end());
  std::vector<int> type_vector(nframes*nall, 0);
  for (int ii = 0; ii < nframes; ++ii) {
    for (int jj = 0; jj < nall; ++jj) {
      type_vector(ii*nall+jj) = datype[jj];
    }
  }

  at::Tensor coord = torch::from_blob(dcoord_.data(), {nframes, nall, 3}, options).to(device);
  at::Tensor type = torch::from_blob(type_vector.data(), {nframes, nall}, int_options).to(device);
  at::Tensor box = torch::from_blob(dbox.data(), {nframes, 9}, options).to(device);
  at::Tensor natoms = torch::from_blob(natoms_vector.data(), {2 + ntypes}, int_options).to(device);
  at::Tensor fparam = torch::from_blob(fparam_.data(), {nframes, fparam_.size()/nframes}, options).to(device);
  at::Tensor aparam = torch::from_blob(aparam_.data(), {nframes, aparam_.size()/nframes}, options).to(device);
  if (b_pbc) {
    std::vector mesh_vector(6, 0);
    at::Tensor mesh = torch::from_blob(mesh_vector.data(), {6}, int_options).to(device);
  }
  else {
    std::vector mesh_vector(0, 0);
    at::Tensor mesh = torch::from_blob(mesh_vector.data(), {0}, int_options).to(device);
  }

  std::vector<VALUETYPE> dcoord(dcoord_);
  atommap.forward<VALUETYPE>(dcoord.begin(), dcoord_.begin(), 3, nframes, nall);

  input_tensors.push_back(coord);
  input_tensors.push_back(type);
  input_tensors.push_back(natoms);
  input_tensors.push_back(mapping);
  input_tensors.push_back(shift);
  input_tensors.push_back(selected);
  input_tensors.push_back(box);
  return nloc;
}
...
void deepmd::read_file_to_module(std::string model, torch::jit::script::Module& pt_module) {
  pt_module = torch::jit::load(model);
}
