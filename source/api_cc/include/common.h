...
template <typename VALUETYPE> class Region3D {
public:
  Region3D();
  ~Region3D() {}
  ...
  Region3D(const std::vector<VALUETYPE> &box) {
    Eigen::Vector3d a1(
        (std::vector<VALUETYPE>(box.begin(), box.begin() + 3)).data());
    Eigen::Vector3d a2(
        (std::vector<VALUETYPE>(box.begin() + 3, box.begin() + 6)).data());
    Eigen::Vector3d a3(
        (std::vector<VALUETYPE>(box.begin() + 6, box.begin() + 9)).data());
    boxt << a1, a2, a3;
  ...
  void phys2inter(std::vector<VALUETYPE> &inner_coord,
                  const std::vector<VALUETYPE> &coord) {
    int natom = coord.size() / 3;
    inner_coord.resize(natom * 3);
    for (int ii = 0; ii < natom; ii++) {
      Eigen::RowVector3d tmp_coord(
          (std::vector<VALUETYPE>(coord.begin() + (3 * ii),
                                  coord.begin() + (3 * (ii + 1))))
              .data());
  ...
  void inter2phys(std::vector<VALUETYPE> &coord,
                  const std::vector<VALUETYPE> &inner_coord) {
    int natom = inner_coord.size() / 3;
    coord.resize(natom * 3);
    for (int ii = 0; ii < natom; ii++) {
      Eigen::RowVector3d tmp_coord(
          (std::vector<VALUETYPE>(inner_coord.begin() + (3 * ii),
                                  inner_coord.begin() + (3 * (ii + 1))))
              .data());
...
void append_neighbors(std::vector<VALUETYPE> &merged_coord_shift,
                      ...
                      Region3D<VALUETYPE> &region, VALUETYPE rcut) {
  int natom = coord.size() / 3;
  a2c.resize(natom);
  int nlocal_region = ncell[0] * ncell[1] * ncell[2];
  int nghost_region = (ncell[0] + 2) * (ncell[1] + 2) * (ncell[2] + 2) -
  ...
  std::vector<std::vector<int>> ghost_region_shift_period_to_inside(
      nghost_region);
  int m = 0;
  for (int ii = -1; ii < ncell[0] + 1; ii++) {
    for (int jj = -1; jj < ncell[1] + 1; jj++) {
      for (int kk = -1; kk < ncell[2] + 1; kk++) {
        if (ii < 0 || ii >= ncell[0] || jj < 0 || jj >= ncell[1] || kk < 0 ||
            kk >= ncell[2]) {
          ...
          for (int ll = 0; ll < 3; ll++) {
            if (mirror_region_id[m][ll] < 0)
              mirror_region_id[m][ll] += ncell[ll];
          }
          mirror_region_serial_id[m] =
              compute_serial_id(mirror_region_id[m], ncell);

          ghost_region_shift_period_to_inside[m].resize(3);
          ghost_region_shift_period_to_inside[m][0] =
              -int(std::floor(VALUETYPE(ii) / VALUETYPE(ncell[0])));
          ghost_region_shift_period_to_inside[m][1] =
              -int(std::floor(VALUETYPE(jj) / VALUETYPE(ncell[1])));
          ghost_region_shift_period_to_inside[m][2] =
              -int(std::floor(VALUETYPE(kk) / VALUETYPE(ncell[2])));
          m++;
        } else {
          std::vector<int> tmp_region_id = {ii, jj, kk};
          region_id_map_serial[tmp_region_id] =
              compute_serial_id(tmp_region_id, ncell);
        }
      }
    }
  }
  ...
  std::vector<int> ghost_atom_ghost_region_idx(
      nghost); // Note: the elements in this vector range from [0, # ghost
               // regions)
  m = 0;
  for (int ii = 0; ii < nghost_region; ii++) {
    std::fill(
        ghost_atom_ghost_region_idx.begin() + m,
        ghost_atom_ghost_region_idx.begin() + (m + nghost_each_region[ii]), ii);
  ...
  for (int ii = 0; ii < nghost; ii++) {
    std::vector<int> inner_int =
        ghost_region_shift_period_to_inside[ghost_atom_ghost_region_idx[ii]];
    std::vector<VALUETYPE> inner_coord_shift(inner_int.begin(),
                                             inner_int.end());
    std::vector<VALUETYPE> shift(3);
    region.inter2phys(shift, inner_coord_shift);
    std::copy(shift.begin(), shift.end(),
              merged_coord_shift.begin() + 3 * (natom + ii));
  }
  ...
  for (int ii = 0; ii < nghost_region; ii++) {
    ...
    std::copy(tmp_mapping.begin(), tmp_mapping.end(),
              merged_mapping.begin() + (natom + last_size));
...
void build_inside_clist(std::vector<int> &a2c,
  ...
  for (int ii = 0; ii < natom; ii++) {
    for (int jj = 0; jj < 3; jj++) {
      cell_offset[3 * ii + jj] =
          std::floor(inner_coord[3 * ii + jj] / inter_cell_size[jj]);
      if (cell_offset[3 * ii + jj] < 0)
        cell_offset[3 * ii + jj] = 0;
    }
  }
  for (int ii = 0; ii < natom; ii++) {
    a2c[ii] =
        compute_serial_id(std::vector<int>(cell_offset.begin() + (3 * ii),
                                           cell_offset.begin() + (3 * ii + 3)),
                          ncell);
...
void build_neighbor_list(
    ...
    const std::vector<VALUETYPE> &coord, const std::vector<int> &atype,
    VALUETYPE rcut, const std::vector<int> &ncell, const std::vector<int> &sec,
  ...
  for (int i = 0; i < nloc; i++) {
    ...
    region_id[1] =
        int((region_serial_id - region_id[0] * ncell[1] * ncell[2]) / ncell[2]);
    region_id[2] = region_serial_id - region_id[0] * ncell[1] * ncell[2] -
                   region_id[1] * ncell[2];
    ...
    for (int ii = -1; ii < 2; ii++) {
      for (int jj = -1; jj < 2; jj++) {
        for (int kk = -1; kk < 2; kk++) {
          // if (! (ii==0 && jj==0 && kk==0)) {
          std::vector<int> neighbor_region_id = {
              region_id[0] + ii, region_id[1] + jj, region_id[2] + kk};
          int neighbor_region_serial_id =
              region_id_map_serial.find(neighbor_region_id)->second;
          neighbor_regions[m] = neighbor_region_serial_id;
          m++;
          // std::vector<int> neighbor_atoms =
          // cell_list[neighbor_region_serial_id];
          // }
        }
      }
    }
    ...
    for (int ii = 0; ii < 27; ii++) {
      neighbor_atoms.insert(neighbor_atoms.end(),
                            cell_list[neighbor_regions[ii]].begin(),
                            cell_list[neighbor_regions[ii]].end());
    }
    ...
    for (int j = 0; j < num_neighbor_atoms; j++) {
      ...
      std::copy(coord.begin() + (3 * i), coord.begin() + (3 * i + 3),
                coord_i.begin());
      std::copy(coord.begin() + (3 * idx_neighbor),
                coord.begin() + (3 * idx_neighbor + 3), coord_j.begin());
      VALUETYPE tmp_distance = std::sqrt(std::pow(coord_j[0] - coord_i[0], 2) +
                                         std::pow(coord_j[1] - coord_i[1], 2) +
                                         std::pow(coord_j[2] - coord_i[2], 2));
    ...
    for (int ii = 0; ii < ntype; ii++) {
      int number_atoms_onetype = std::count(neighbor_atype_in_rcut.begin(),
                                            neighbor_atype_in_rcut.end(), ii);
      // neighbor_atoms_each_type[ii] =
      // std::count(neighbor_atype_in_rcut.begin(),
      // neighbor_atype_in_rcut.end(), ii);
      ...
      std::sort(indices.begin(), indices.end(),
                [&neighbor_distances_onetype](size_t i1, size_t i2) {
                  return neighbor_distances_onetype[i1] <
                         neighbor_distances_onetype[i2];
                });
      ...
      for (int jj = 0; jj < indices.size(); jj++) {
        ...
        if (jj < sec_[ii]) {
          // if (i==14) { printf("ii = %d jj = %d sec_offset = %d i = %d %d %d
          // %d\n", ii, jj, sec_offset[ii], i, sec_offset[ii]+jj, indices[jj],
          // neighbor_atoms_onetype[indices[jj]]); }
          ...
          nlist_loc[i][sec_offset[ii] + jj] =
              merged_mapping[neighbor_atoms_onetype[indices[jj]]];
        }
      }
      // printf("\n");
    ...
    // std::cout << std::count(neighbor_atoms_atype.begin(),
    // neighbor_atoms_atype.end(), 0) << " " <<
    // std::count(neighbor_atoms_atype.begin(), neighbor_atoms_atype.end(), 1)
    // << std::endl; std::cout << neighbor_atoms.size() << std::endl;
  }
...
void make_env_mat(std::vector<std::vector<int>> &nlist,
                  ...
                  const std::vector<VALUETYPE> &coord,
                  const std::vector<int> &atype,
                  const std::vector<VALUETYPE> &box, VALUETYPE rcut,
  ...
  append_neighbors(merged_coord_shift, merged_atype, merged_mapping, a2c,
                   region_id_map_serial, cell_list, ncell, coord_wrapped, atype,
                   region, rcut);

  //   for (std::map<std::vector<int>, int>::iterator
  //   iter=region_id_map_serial.begin(); iter!=region_id_map_serial.end();
  //   iter++){
  ...
  for (int ii = 0; ii < nall; ii++) {
    ...
    for (int jj = 0; jj < 3; jj++) {
      ...
      merged_coord[3 * ii + jj] =
          coord_wrapped[3 * idx + jj] - merged_coord_shift[3 * ii + jj];
    }
  }
  //   for (int ii=0; ii<nall; ii++) {
  //     printf("%.4f %.4f %.4f\n", merged_coord[3*ii+0], merged_coord[3*ii+1],
  //     merged_coord[3*ii+2]);
  //   }
  int nloc = atype.size();
  build_neighbor_list(nlist, nlist_loc, nlist_type, nloc, merged_coord,
                      merged_atype, rcut, ncell, sec, merged_mapping, a2c,
                      region_id_map_serial, cell_list);
